...
echo "DeltaAegis v0.44 Stage 1-2 Gate"
echo "================================"
...
if [[ "$allow_dirty" -eq 0 ]]; then
  if [[ -n "$(git status --short)" ]]; then
    echo "ERROR: v0.44 stage 1-2 gate requires a clean working tree" >&2
    git status --short >&2
    exit 1
  fi
...
bash -n tools/validate_v0_44_stage1_2_all.sh
...
echo
echo "PASS: DeltaAegis v0.44 stage 1-2 gate"
