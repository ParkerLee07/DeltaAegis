...
echo "DeltaAegis v0.44 Stage 3 Gate"
echo "================================"
...
if [[ "$allow_dirty" -eq 0 ]]; then
  if [[ -n "$(git status --short)" ]]; then
    echo "ERROR: v0.44 stage 3 gate requires a clean working tree" >&2
    git status --short >&2
    exit 1
  fi
...
echo "[v0.44 stage 3] predecessor behavior compatibility"
python3 tools/validate_v0_43_v0_42_compatibility.py

echo
echo "PASS: DeltaAegis v0.44 stage 3 gate"
